...
- **Client Components** (the contact form, the footer newsletter form) POST to `/api/contact` (`src/app/api/contact/route.ts`) — a custom route that layers rate limiting, honeypot/timing spam defense, and email notifications on top of a Local API `create` call, rather than hitting Payload's auto-generated `/api/leads` REST endpoint directly (that endpoint still exists and still works, it's just not what the UI uses anymore). The only client-side data-fetching in the app.
