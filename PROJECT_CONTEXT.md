...
- **Content collections are public-read, admin-write; Leads is the inverse.** Anyone can `create` a Lead (the contact form is public), but only logged-in admins can `read`/`update`/`delete` leads or write content. See `src/collections/access.ts`.
...
| Leads | `leads` | Contact form submissions | `name`, `email`, `phone`, `company`, `businessType` (select), `interestedServices` (select, hasMany — real service categories), `monthlyAdBudget` (select), `message`, `preferredContactMethod` (select), `status` (select, sidebar), `source` (UTM group, sidebar) |
...
Pattern: content collections are `read: anyone, create/update/delete: isLoggedIn`. Leads is `create: anyone, read/update/delete: isLoggedIn`. Users is `isLoggedIn` on everything (Payload's built-in first-user bootstrap bypasses this for the very first account).
...
- **Client Components** (the contact form, the footer newsletter form) POST to Payload's REST API at `/api/leads` — the only client-side data-fetching in the app.
- **Seeding:** `src/seed.ts` populates Services/Pricing/FAQs/sample Testimonials/sample CaseStudies. Run via `node --env-file=.env --import tsx src/seed.ts` (needs `CI=true` prefix if Drizzle's schema-push prompts interactively — see §11). It's idempotent-ish: checks `services` count and skips if already seeded.
...
- Server-side rate limiting (Upstash Redis was the planned approach)
- Email notifications: confirmation to the lead, alert to the admin (Resend + react-email was the planned approach) — currently leads only get written to Postgres, no email fires
